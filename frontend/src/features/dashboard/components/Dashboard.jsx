...
import { Notecontext, useNotification, SearchInput, config } from '../../../shared'
...
function Dashboard() {
  ...
  }, [navigate, handleGeneralNotificationClick]);
  ...
  const setLostItems = useCallback(() => {
    setFilterItem('lost');
    setIsSearchMode(false);
  }, []);

  const setFoundItems = useCallback(() => {
    setFilterItem('found');
    setIsSearchMode(false);
  }, []);
  ...
  useEffect(() => {
    getData();
  }, []); // Remove getData dependency to prevent infinite loop
      ...
      <div className="max-w-[85rem] min-h-min mx-auto py-10 px-4 sm:px-6 lg:px-8">
        <div className='lg:flex lg:justify-between lg:items-center'>
          ...
          <div className='flex flex-col gap-2 sm:flex-row sm:m-3 '>
            <button onClick={setLostItems} className="button1">
              <span className="button_lg">
                <span className="button_sl"></span>
                <span className="button_text">Lost Items</span>
              </span>
            </button>
            <button onClick={setFoundItems} className="button1">
              <span className="button_lg">
                <span className="button_sl"></span>
                <span className="button_text">Found Items</span>
              </span>
            </button>
            <button onClick={setAllItems} className="button1">
              <span className="button_lg">
                <span className="button_sl"></span>
                ...
                filteredProducts.length > 0 ? (
                  filteredProducts.map((product) => {
                    ...
                    if (product.type === 'lost') {
                      ...
                      return (
                        <FoundItemCard key={product._id} id={product._id} {...product} imageUrl={imageUrl} />
                      )
                    }
                    return null;
                  })
